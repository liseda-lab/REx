...
<p align="center">
  <img src="Main_Overview.png" alt="REx Overview" width="800"/>
</p>
...
## The following sections are automatic step in the new pytorch version, but for the original version you will need to run the following steps after running REx to get the final results.
...
The `compute_IC` folder contains a guide and template scripts for calculating the IC for another dataset. However, to run REx you can have your own scores of relevance in the same format as the `clustered_IC_classes_edgeType.json`. The scripts in the `compute_IC` folder are as follows:
